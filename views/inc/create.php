...
<form action="/inc/store" method="POST">
   ...
   <!-- Assessment & Planning -->
    <div class="bg-white shadow-md rounded-lg mb-6">
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="catatan_khusus" name="catatan_khusus" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" placeholder="Catatan khusus selama persalinan..."></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-end">
        <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-lg font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            <i data-lucide="save" class="mr-2 h-5 w-5"></i>Simpan Data Persalinan
        </button>
    </div>
</form>
<script>
document.getElementById('pasien_id').addEventListener('change', function() {
    ...
    if (pasienId) {
        fetch(/inc/get_patient_data?pasien_id=)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Show gestational age in card
                    if (data.usia_kehamilan !== null) {
                        document.getElementById('card-usia-kehamilan').textContent = data.usia_kehamilan + ' minggu';
                    } else {
                        document.getElementById('card-usia-kehamilan').textContent = '-';
                    }
                } else {
